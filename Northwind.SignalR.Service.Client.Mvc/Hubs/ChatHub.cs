...
public class ChatHub : Hub
{
    ...
    // so we must store user names, connection IDs, and gropus in a static field
    ...
    public async Task Register(UserModel newUser)
    {
        ...
        if (Users.ContainsKey(newUser.Name))
        {
            ...
            // remove any existing gropu registrations
    ...
    public async Task SendMessage(MessageModel message)
    {
        ...
        foreach (string userOrGroup in userAndGroupList)
        {
            ...
            {
                // if the item is in Users then send the message to that user by looking up their connection ID in the dictionary
